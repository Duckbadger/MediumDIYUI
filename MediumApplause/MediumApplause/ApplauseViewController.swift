...
final class ApplauseViewController: UIViewController {
	...
	fileprivate func updateButtonImage() {
      ...
      applauseButton.setImage(#imageLiteral(resourceName: "empty"), for: .highlighted)
		} else {
...
extension ApplauseViewController {
  ...
  @objc fileprivate func animateIncrement() {
    ...
    let buttonAnimations = applauseButtonAnimationActions
    
    // Animate applause amount view and increment
    let shouldStartFresh = (applauseCounterView.layer.frame == applauseCounterViewInitialFrame || applauseCounterView.layer.frame.midY < applauseCounterViewVisibleFrame.midY)
    let counterAnimations = shouldStartFresh ? applauseCounterAnimationActionsInitial : applauseCounterAnimationActions
    
    let sparkleAnimations = sparklesAnimationActions
    
    let allActions = ActionGroup(actions: [buttonAnimations, counterAnimations, sparkleAnimations])
